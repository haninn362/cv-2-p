...
# ======================================================
# SECTION 2 — Final + Sensibilité (Streamlit adaptation)
# ======================================================
...
# --------- OUTILS AFFICHAGE ---------
def _disp_streamlit(obj, n=None, title=None):
    if title:
        st.subheader(title)
    if isinstance(obj, pd.DataFrame):
        st.dataframe(obj.head(n) if n else obj, use_container_width=True)
    else:
        st.write(obj)

# ======================================================
# PARTIE A : Q* (Qr*, Qw*, n*) depuis fichier Excel
# ======================================================
...
def compute_qstars(file_like, codes: list):
    xls = pd.ExcelFile(file_like)
    df_conso = pd.read_excel(xls, sheet_name="consommation depots externe")
...
# ======================================================
# PARTIE B : Rolling logic (simplified adaptation)
# ======================================================
...
# ======================================================
# PARTIE C : Interface Streamlit
# ======================================================
st.header("📊 Final + Sensibilité")

file_data = st.file_uploader("Téléverser le fichier Excel (PFE HANIN)", type=["xlsx", "xls"], key="final_file")
file_ses = st.file_uploader("Paramètres SES", type=["xlsx"], key="ses_file")
file_cro = st.file_uploader("Paramètres Croston", type=["xlsx"], key="cro_file")
file_sba = st.file_uploader("Paramètres SBA", type=["xlsx"], key="sba_file")
...
if file_data and file_ses and file_cro and file_sba:
    if st.button("▶️ Lancer l'analyse"):
        best_per_code = select_best_method_from_files(
            path_ses=file_ses, path_cro=file_cro, path_sba=file_sba
        )
        _disp_streamlit(best_per_code, title="✅ Meilleure méthode et meilleurs paramètres par article")

        final_95 = run_final_once(best_per_code, service_level=NIVEAU_SERVICE_DEF)
        _disp_streamlit(final_95[COLONNES_AFFICHAGE], n=20, title="Résultats SL 95%")
        ...
        if not sensi.empty:
            ...
            _disp_streamlit(summary, title="📊 Résumé global – moyennes par code et niveau de service")
