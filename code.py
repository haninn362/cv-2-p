...
# (adaptation du script unifié pour Streamlit)
# ============================================
...
def _daily_consumption_and_stock(excel_path, sheet_name):
...
# ---------- SBA, Croston, SES core loops ----------
# (for brevity we reuse the same structure, but with different forecast functions)

def _sba_forecast(x, alpha):
    ...
    first = nz_idx[0]
    z = x[first]
    p = len(x) / len(nz_idx) if len(nz_idx) < 2 else sum([j - i for i, j in zip(nz_idx[:-1], nz_idx[1:])]) / len(nz_idx)
    psd = 0
...
# Wrapper to run one method
def _rolling_method(excel_path, code, alpha, window_ratio, interval, forecast_func):
    ...
    for i in range(split_index, len(vals)):
        if (i - split_index) % interval == 0:
            ...
            X_Lt = LEAD_TIME_UNI * f
            sigma_Lt = sigma_period * np.sqrt(max(LEAD_TIME_UNI, 1e-9))
            var_u = sigma_Lt**2 if sigma_Lt**2 > X_Lt else X_Lt + 1e-5
            p_nb = min(max(X_Lt / var_u, 1e-12), 1 - 1e-12)
            r_nb = X_Lt**2 / (var_u - X_Lt) if var_u > X_Lt else 1e6
            ROP_u = float(np.percentile(nbinom.rvs(r_nb, p_nb, size=NB_SIM_UNI, random_state=rng), 100 * SERVICE_LEVEL_UNI))

            totalL = LEAD_TIME_UNI + LEAD_TIME_SUPPLIER_UNI
            X_Lt_Lw = totalL * f
            ...
            ROP_f = float(np.percentile(nbinom.rvs(r_nb_f, p_nb_f, size=NB_SIM_UNI, random_state=rng), 100 * SERVICE_LEVEL_UNI))
            ...
            rows.append({
                ...
                "lead_time_usine_days": int(LEAD_TIME_UNI),
                "lead_time_supplier_days": int(LEAD_TIME_SUPPLIER_UNI),
...
if uploaded is not None:
    ...
    for method_name, forecast_func in [
            ...
            for a in ALPHAS_UNI:
                for w in WINDOW_RATIOS_UNI:
                    for itv in RECALC_INTERVALS_UNI:
                        df_run = _rolling_method(EXCEL_PATH_UNI, code, a, w, itv, forecast_func)
                        if df_run.empty:
                            continue
                        est = df_run["reorder_point_usine"] / df_run["lead_time_usine_days"].replace(0, np.nan)
                        e = df_run["real_demand"] - est
                        RMSE = float(np.sqrt((e**2).mean())) if not e.empty else np.nan
        ...
        for _, r in df_best.iterrows():
            df_final = _rolling_method(EXCEL_PATH_UNI, r["code"], r["alpha"], r["window_ratio"], r["recalc_interval"], forecast_func)
            st.markdown(f"**{method_name} — Table finale pour {r['code']}**")
            st.dataframe(df_final[DISPLAY_COLUMNS_UNI], use_container_width=True)

else:
    st.info("Veuillez téléverser un fichier Excel pour lancer les prévisions SBA, Croston et SES.")
